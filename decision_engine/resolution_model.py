...
def _finalize_plan(
    ...
    can_auto_resolve = bool(
        known_cause
        and root_cause_confidence >= 0.70
        and (
            (plan.can_auto_execute and plan.resolution_confidence >= 0.55)
            or (failure_probability >= 0.90 and plan.resolution_confidence >= 0.70)
        )
    )
...
def recommend_resolution(
    ...
    try:
        ...
        root_cause_confidence = _bounded(_safe_float(root_cause_confidence, 1.0))
        ...
        if root == "CPU_OVERLOAD":
            plan = ResolutionPlan(
                ...
                can_auto_execute=severity >= 0.45,
            ...
            plan = ResolutionPlan(
                ...
                can_auto_execute=severity >= 0.48,
                resolution_status="PENDING",
                resolution_alert="",
            )
            ...
            plan = ResolutionPlan(
                ...
                can_auto_execute=severity >= 0.50,
                resolution_status="PENDING",
                resolution_alert="",
            )
        else:
